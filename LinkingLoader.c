...
void Loader(int argc, char argv[][ARGV_MAX_LEN]){
    ...
    err_flag=LoadPassOne(fp_arr,argc-1);
    if(err_flag<0){
        EraseEstab(argc-1);
        historypop();
        return;
    ...
    err_flag=LoadPassTwo(fp_arr,argc-1);
    if(err_flag<0){
        EraseEstab(argc-1);
        historypop();
        return;
...
int LoadPassTwo(FILE* fp_arr[], int filenum){
    ...
    for(int file_i=0;file_i<filenum;file_i++){
        ...
        ref_num_arr[1]=Estab[file_i]->address;
        //printf("\n%dth file csaddr: %X\n\n",file_i,csaddr);
        while(fgets(object_file_input,OBJ_MAX_LEN,fp_arr[file_i])!=NULL){
            int input_len=strlen(object_file_input);
            if(input_len!=0)
            ...
            if(record_letter=='R'){
                ...
                for(int i=1;i<input_len;i+=8){
                    ...
                    ref_num_arr[ref_num]=node->address;
                ...
                ref_address=ref_num_arr[idx];
                //printf("ref_address : %c%X\n",ref_str[0],ref_address);
